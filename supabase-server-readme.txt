...
> **v1.X ΓÇö Public Beta.** First stable release under SemVer: breaking changes only ship as a major bump. The package is still early ΓÇö expect new adapters, ergonomic improvements, and features to land frequently in minor releases. Found a rough edge? [Open an issue](https://github.com/supabase/server/issues) or [submit a PR](https://github.com/supabase/server/blob/main/CONTRIBUTING.md).

> **Coming from a `0.x` release?** See [MIGRATION.md](MIGRATION.md) for the v0 ΓåÆ v1 rename map (`allow` ΓåÆ `auth`, `'public'` ΓåÆ `'publishable'`, `authType` ΓåÆ `authMode`, `claims` ΓåÆ `jwtClaims`, ΓÇª).
...
export default {
  fetch: withSupabase({ auth: 'user' }, async (_req, ctx) => {
    // RLS-scoped ΓÇö this user only sees their own favorites
...
```bash
# Deno / Supabase Edge Functions (no install ΓÇö import directly)
...
export default {
  fetch: withSupabase({ auth: 'user' }, async (_req, ctx) => {
    ...
    // supabase       ΓÇö RLS-scoped to the authenticated user
    // supabaseAdmin  ΓÇö bypasses RLS (service role)
    // userClaims     ΓÇö user identity from JWT (id, email, role)
    // jwtClaims      ΓÇö full JWT claims
    // authMode       ΓÇö which auth mode matched
...
// auth: 'publishable' validates the apikey header against a publishable key ΓÇö
...
export default {
  fetch: withSupabase({ auth: 'publishable' }, async (_req, ctx) => {
    // ctx.supabase  ΓÇö anonymous (anon role); RLS still applies
    // ctx.userClaims, ctx.jwtClaims ΓÇö null (no JWT)
...
> Unlike `auth: 'secret'`, the `supabase` client here is anonymous, not admin ΓÇö RLS is the source of truth for what's visible. The publishable key acts as a coarse "this request came from a known client" gate; it isn't a user identity.

### API key protected

```ts
// An admin dashboard fetches the list of featured games to curate.
// Secret key auth (not a user JWT) ΓÇö supabaseAdmin bypasses RLS.
export default {
...
export default {
  fetch: withSupabase({ auth: ['user', 'secret'] }, async (req, ctx) => {
    ...
    if (callerIsUser) {
      // RLS-scoped ΓÇö the database enforces "own stats only"
      const { data: myStats } = await ctx.supabase.from('play_stats').select()
      return Response.json(myStats)
    }

    // Service path ΓÇö bypass RLS to pull stats for any user
...
Array syntax (`auth: ["user", "secret"]`) accepts multiple auth methods ΓÇö first match wins. An absent credential falls through to the next mode; a present-but-invalid JWT rejects the request (no silent downgrade). See [`docs/auth-modes.md`](docs/auth-modes.md).
...
`supabase` is always the safe client ΓÇö it respects RLS. When `authMode` is `"user"`, it's scoped to that user's permissions. Otherwise, it's initialized as anonymous.
...
Adapters wrap `withSupabase` for a specific framework's middleware contract. They ship inside `@supabase/server`, so a single `npm install @supabase/server` covers the framework you're using ΓÇö no separate package per adapter.

> **Adapters are a community-driven initiative.** They're developed, maintained, and evolved by contributors ΓÇö including responding to upstream framework changes. See [`src/adapters/README.md`](src/adapters/README.md) for the contribution requirements (tests, types, docs, build wiring) if you'd like to add or help maintain one.
...
const app = new Elysia()
  // Protected ΓÇö plugin resolves supabaseContext before handlers run
  ...
  // Public ΓÇö no plugin means no auth
...
The adapter does not handle CORS ΓÇö use `@elysiajs/cors` for that.
...
For when you need more control than `withSupabase` provides ΓÇö multiple routes with different auth, custom response headers, or building your own wrapper.
...
Low-level ΓÇö works with raw credentials instead of a Request. Used by SSR adapters and custom auth flows.
...
Full context assembly from a Request ΓÇö `verifyAuth` + client creation in one call.
...
export default {
  fetch: async (req) => {
    ...
    // Public ΓÇö no auth needed
    if (url.pathname === '/health') {
      return Response.json({ status: 'ok' })
    }

    // Protected ΓÇö verify the JWT, then create a user-scoped client
...
`@supabase/server` runs anywhere standard Web `fetch` does ΓÇö pick the row that matches your deployment target.

| Target                      | Notes                                                                                                                                     |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| **Supabase Edge Functions** | Zero config ΓÇö environment variables are auto-injected.                                                                                    |
